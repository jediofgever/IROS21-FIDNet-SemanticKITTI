...
if __name__ == '__main__':  # an example for generating video of sequence 00
    data_path = 'sequences/00/'
    videoWriter = cv.VideoWriter('00.avi', cv.VideoWriter_fourcc(
        'M', 'J', 'P', 'G'), 10, (1024, 320), True)
    for bin_file, label_file, tag_file in zip(sorted(os.listdir(data_path+'velodyne/')),
        ...
        rangeimage = get_rangeimage(
            data_path+'velodyne/'+bin_file, data_path+'tag/'+tag_file)
        rangeimage = cv.resize(rangeimage, (1024, 128))
        whiteimage = np.ones((64, 1024, 3), dtype=np.uint8)*255
        semimage = get_semimage(data_path+'labels/' +
                                label_file, data_path+'tag/'+tag_file)
        semimage = cv.resize(semimage, (1024, 128))
        mergeimage = np.concatenate((rangeimage, whiteimage, semimage), axis=0)
