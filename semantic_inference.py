...
parser.add_argument('--root',  dest="root", default='/home/atas/poss_data/',
                    help="/home/atas/poss_data/")
...
dataset_train = POSSDataset(root=args.root, split='test', is_train=True, range_img_size=(
    args.range_y, args.range_x), if_aug='True', if_range_mask=args.if_range_mask, if_remission=args.if_remission, if_range=args.if_range, with_normal=args.with_normal)
...
save_path_for_prediction = "./method_predictions/sequences/03/"
if not os.path.exists(save_path_for_prediction):
    os.mkdir(save_path_for_prediction)

save_path_for_prediction = "./method_predictions/sequences/03/predictions/"
if not os.path.exists(save_path_for_prediction):
    os.mkdir(save_path_for_prediction)
...
for i in range(len(lidar_list)):
    ...
    if args.with_normal:
        ...
        normal_image = normal_image * \
            np.transpose([A.proj_mask, A.proj_mask, A.proj_mask], [1, 2, 0])
    ...
    if args.if_KNN == 1:
        t_1 = torch.squeeze(range_img*80.0).detach().to(device)
        t_2 = torch.squeeze(FF.to_tensor(np.reshape(
            A.unproj_range, (1, -1)))).detach().to(device)
        t_3 = torch.squeeze(semantic_pred).detach().to(device)
        t_4 = torch.squeeze(FF.to_tensor(np.reshape(
            A.proj_x, (1, -1)))).to(dtype=torch.long).detach().to(device)
        t_5 = torch.squeeze(FF.to_tensor(np.reshape(
            A.proj_y, (1, -1)))).to(dtype=torch.long).detach().to(device)
